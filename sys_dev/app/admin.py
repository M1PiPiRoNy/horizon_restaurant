...
#     def __init__(self, master):
#         self.master = master
#         self.stock_check_window = tk.Toplevel(master)
#         self.stock_check_window.title("Stock Check Page")

#         # Set a larger default size for the Stock Check Page
#         self.stock_check_window.geometry("600x400")

#         # Create a frame for stock checking
#         check_frame = ttk.Frame(self.stock_check_window, padding="20")
#         check_frame.pack()

#         # Create labels and entry widgets for stock checking
#         ttk.Label(check_frame, text="Item Name:").grid(row=0, column=0, sticky=tk.W)
#         self.check_item_name_entry = ttk.Entry(check_frame)
#         self.check_item_name_entry.grid(row=0, column=1, pady=5)

#         # Create a button to search specific item
#         search_button = ttk.Button(check_frame, text="Search", command=self.search_stock)
#         search_button.grid(row=2, column=0, columnspan=2, pady=10)

#         # Create a text widget to display stock information
#         self.stock_text = tk.Text(self.stock_check_window, wrap=tk.WORD, font=('Helvetica', 12), height=20, width=70, bg="white")
#         self.stock_text.pack(pady=20, padx=20)
...
#         item_name = self.check_item_name_entry.get()

#         if item_name:
#             # Search stock in the database
#             stock_quantity = self.get_stock_quantity(item_name)
#             tk.messagebox.showinfo("Stock Check", f"Stock quantity for '{item_name}': {stock_quantity}.")
#             self.stock_check_window.destroy()
#         else:
#             tk.messagebox.showwarning("Error", "Please enter a valid item name for stock checking.")
...
#             cursor.execute("CREATE TABLE IF NOT EXISTS stock_orders (item_name TEXT, quantity INTEGER)")
#             cursor.execute("SELECT SUM(quantity) FROM stock_orders WHERE item_name=?", (item_name,))
#             stock_quantity = cursor.fetchone()[0]

#             connection.close()

#             return stock_quantity if stock_quantity else 0
#         except Exception as e:
#             tk.messagebox.showerror("Error", str(e))
#             return 0
#nour
